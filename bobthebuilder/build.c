...
#define LOG(level, ...)                               \
    ...
    {                                                 \
        int err;                                      \
        if ((err = __refresh_date()))                 \
            return err;                               \
        switch (level)                                \
        {                                             \
        case LOG_LEVEL_INFO:                          \
        {                                             \
            fprintf(stdout, "[%s INFO]: ", datebuf);  \
            fprintf(stdout, __VA_ARGS__);             \
        };                                            \
        break;                                        \
        case LOG_LEVEL_ERROR:                         \
        {                                             \
            fprintf(stderr, "[%s ERROR]: ", datebuf); \
...
#define __C_APPEND(c, str)                       \
    do                                           \
        if ((err = command_append_arg(&c, str))) \
            return err;                          \
    while (0);
...
static const char *executable_path = "./src/bobthebuilder.c";
...
static int __build_executable(char debug)
{
    ...
    __C_APPEND(c, "gcc ");
    __C_APPEND(c, "./src/bobthebuilder.c ");
    __C_APPEND(c, "-o ");
    __C_APPEND(c, "./dist/bobthebuilder ");
    __C_APPEND(c, "-Wall ");
    __C_APPEND(c, "-Wextra ");
    __C_APPEND(c, "-Werror ");
